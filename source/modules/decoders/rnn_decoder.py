...
class RNNDecoder(nn.Module):
    ...
    def forward(self, inputs, state):
        ...
        num_valid_list = sequence_mask(sorted_lengths).int().sum(dim=0)#num_valid_list的维度（max_len),每一维代表一个时间步的输入的batch的数量
        ...
        for i, num_valid in enumerate(num_valid_list):
            ...
            state.hidden[:, :num_valid] = valid_state.hidden#更新当前时间步的隐层状态
            out_inputs[:num_valid, i] = out_input.squeeze(1)#收集当前时间步的输出，直到所有的收集完。

        # Resort撤回排序
        _, inv_indices = indices.sort()
        state = state.index_select(inv_indices)#state是个字典，包括hidden、query、memory等
        out_inputs = out_inputs.index_select(0, inv_indices)

        log_probs = self.output_layer(out_inputs)#维度(batchsize,max_len,词表长度)
