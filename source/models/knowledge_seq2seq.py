...
class KnowledgeSeq2Seq(BaseModel):
    ...
    def encode(self, inputs, hidden=None, is_training=False):
	    ...
	    #{'src':( 数据值-->shape(batch_size,max_len), 数据长度值--> shape(batch_size) ),
	      ...
	      'cue' :( 数据值-->shape(batch_size,sent_num,max_len), 数据长度值--> shape(batch_size,cue_num))}
	    '''
        ...
        if self.use_gs:
            knowledge = cue_outputs.gather(1, \
                                           indexs.view(-1, 1, 1).repeat(1, 1, cue_outputs.size(
                                               -1)))  # (batch_size, 1 , 2*rnn_hidden_size)
        else:
            knowledge = weighted_cue

        if self.use_posterior:
            tgt_enc_inputs = inputs.tgt[0][:, 1:-1]
            inputs.tgt[1] = inputs.tgt[1] - 2
            _, tgt_enc_hidden = self.knowledge_encoder(tgt_enc_inputs,
                                                       hidden)  # tgt_enc_hidden:(num_layer , batch_size , 2*rnn_hidden_size)
            ...
            if self.use_gs:
                gumbel_attn = F.gumbel_softmax(torch.log(cue_attn + 1e-10), 0.1,
                                               hard=True)  # cue_attn:(batch_size, cue_num)
                knowledge = torch.bmm(gumbel_attn.unsqueeze(1),
                                      cue_outputs)  # cue_outputs:(batch_size, cue_num, 2 * rnn_hidden_size) knowledge(batch_size, 1 , 2*rnn_hidden_size)
                indexs = gumbel_attn.max(-1)[1]
            else:
                knowledge = weighted_cue
